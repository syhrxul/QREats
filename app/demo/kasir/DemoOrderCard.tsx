...
interface DemoOrderCardProps {
  order: Order;
  newOrderId: string | null;
  onClick: () => void;
}

export default function DemoOrderCard({ order, newOrderId, onClick }: DemoOrderCardProps) {
    ...
    >
      ...
      <div className="mb-2.5">
        <div className="flex items-center gap-2 mb-1">
          {newOrderId === order.id && (
            <span className="text-[10px] bg-amber-500 text-white font-bold px-2 py-0.5 rounded-full animate-pulse">BARU</span>
          )}
          <span className="font-bold text-slate-900 text-lg">{order.table_number}</span>
          {order.customer_name && (
            <span className="text-sm font-semibold text-slate-900/60">({order.customer_name})</span>
          )}
        </div>
      ...
      <div className="flex items-center justify-between pt-3 border-t border-slate-900/5">
        ...
        <div className="flex items-center gap-2">
          {order.receipt_path && (
            <span className="text-[11px] font-bold text-amber-600 bg-amber-50 px-2.5 py-1 rounded-lg">Bukti Transfer</span>
          )}
