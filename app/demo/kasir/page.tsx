  ...
  return (
    <div className="min-h-screen bg-slate-50 font-sans">
      <div className="bg-gradient-to-r from-orange-500 to-amber-500 text-white px-6 py-3 flex items-center justify-between">
        ...
        <Link href="/" className="bg-white text-indigo-600 hover:bg-white/90 text-xs font-bold px-4 py-2 rounded-xl transition-all shadow">← Kembali ke Beranda</Link>
      </div>

      {/* Header */}
      <div className="border-b border-slate-900/10 px-6 py-4 flex items-center justify-between bg-white">
        <div>
          <h2 className="text-lg font-bold text-slate-900">Daftar Antrean Order · Kafe Demo</h2>
        ...
        <div className="flex items-center gap-3">
          ...
          <button
            ...
            className={`flex items-center gap-1.5 text-xs px-3 py-1.5 rounded-full border transition-all font-medium cursor-pointer ${
              soundEnabled
                ? 'bg-indigo-50 border-indigo-200 text-orange-700 hover:bg-indigo-100'
              ...
              return (
                ...
                >
                  <div className="absolute top-5 right-5 flex items-center gap-2">
                    ...
                    {order.is_ready ? (
                      ...
                      <span className="text-[10px] font-bold px-2.5 py-1 rounded-lg bg-slate-900/5 text-slate-900/60 border border-slate-900/10">SEDANG DIBUAT</span>
                    )}
                ...
                <div className="mb-2.5">
                  <div className="flex items-center gap-2 mb-1">
                    ...
                    {order.customer_name && (<span className="text-sm font-semibold text-slate-900/60">({order.customer_name})</span>)}
                  </div>
                  <p className="text-xs text-slate-900/40">Jam: {formatTime(order.created_at)} · ID: #{order.id.slice(0, 8)}</p>
                </div>
                ...
                <div className="flex items-center justify-between pt-3 border-t border-slate-900/5">
                  <div>
                    ...
                    <p className="text-lg font-bold text-slate-900">{formatRupiah(order.total_price)}</p>
                  </div>
          ...
          <div className="absolute inset-y-0 right-0 max-w-full flex">
            <div className="w-screen max-w-md bg-[#F9F6EE] shadow-[4px_4px_0px_0px_rgba(15,23,42,1)] border-2 border-slate-900 flex flex-col justify-between border-l border-slate-900/10">
              <div className="px-6 py-5 bg-white border-b border-slate-900/10 flex items-center justify-between">
                ...
                <button onClick={() => setSelectedOrder(null)} className="w-8 h-8 rounded-full bg-slate-900/10 flex items-center justify-center text-slate-900 hover:bg-slate-900/20 transition-colors">✕</button>
              </div>
                ...
                {selectedOrder.receipt_path && (
                  <div>
                    ...
                    <div className="bg-white border border-slate-900/10 rounded-xl p-6 text-center">
                      <div className="w-20 h-20 bg-slate-50 rounded-xl flex items-center justify-center text-3xl mx-auto mb-2">📸</div>
              ...
              <div className="p-6 border-t border-slate-900/10 bg-white space-y-3">
                ...
                {selectedOrder.status === 'paid' && (
                  <button onClick={() => handleToggleReady(selectedOrder.id)} className={`w-full py-3.5 text-sm font-bold rounded-xl active:scale-[0.98] transition-all ${selectedOrder.is_ready ? 'bg-slate-900/10 text-slate-900 hover:bg-slate-900/15' : 'bg-emerald-600 text-white hover:bg-emerald-700'}`}>
      ...
      <div className="fixed bottom-6 right-6 z-50 space-y-2 pointer-events-none max-w-sm w-full">
        {toasts.map((t) => (
          <div
            key={t.id}
            className="bg-slate-900 text-white border border-white/10 rounded-xl p-4 shadow-[4px_4px_0px_0px_rgba(15,23,42,1)] border-2 border-slate-900 flex items-center justify-between gap-4 pointer-events-auto animate-fade-in"
          >
            <div className="flex items-center gap-3">
