...
export default function Sidebar({
  ...
  impersonateShopName,
  handleLogout,
}: Omit<SidebarProps, 'isMobileOpen' | 'setIsMobileOpen'>) {
      ...
      <div className="px-6 py-6 border-b border-white/10 flex items-center gap-3">
        ...
        <div className="min-w-0 flex-1">
          ...
          <span className="text-[9px] text-[#FF9F43] font-bold tracking-wider uppercase">
            {isSuperadmin ? (impersonateShopId ? 'Preview Mode' : 'Superadmin') : 'Merchant Admin'}
          </span>
        </div>
      </div>
      ...
      {shop && !isSuperadmin && (
        <div className="mx-4 mb-3 p-3 bg-white/5 rounded-2xl border border-white/5 text-xs">
          ...
          <div className="w-full bg-white/10 h-1 rounded-full mt-2 overflow-hidden">
            <div
              className={`h-full rounded-full ${daysLeft <= 3 ? 'bg-rose-500' : 'bg-emerald-500'}`}
      ...
      <div className="p-4 border-t border-white/5 bg-black/20 space-y-3">
        {profile && (
          <div className="px-1 space-y-1">
            <span className="text-[9px] text-white/40 font-bold uppercase tracking-wider block">Pengguna Aktif</span>
        ...
        <button
          ...
          className="w-full flex items-center justify-center gap-2 px-3 py-2.5 border border-white/15 text-white/80 rounded-xl text-xs font-bold hover:bg-white/5 active:scale-[0.98] transition-all cursor-pointer"
        >
