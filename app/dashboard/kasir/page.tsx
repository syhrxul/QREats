...
export default function KasirDashboardPage() {
  ...
  // ─── Supabase Realtime (Deteksi order masuk secara instan) ────────────────────
      ...
      .on(
        ...
        async (payload: any) => {
          if (payload.eventType === 'INSERT') {
            ...
            // filter manual multi-tenant jika bukan superadmin
            ...
            // Hilangkan highlight setelah 6 detik
            setTimeout(() => {
              setNewOrderIds((prev) => {
            ...
            // Update juga drawer jika sedang melihat order yang bersangkutan
  ...
  async function handleRejectPrompt(orderId: string) {
    ...
    const { error } = await supabase
      ...
      .update({ 
        status: 'rejected',
        receipt_path: cleanReason
      })
      .eq('id', orderId);

    if (!error) {
      setOrders((prev) =>
        prev.map((o) => (o.id === orderId ? { ...o, status: 'rejected', receipt_path: cleanReason } : o))
      );
      
      setSelectedOrder((prev) => {
        if (prev && prev.id === orderId) {
          return { ...prev, status: 'rejected', receipt_path: cleanReason };
        }
        return prev;
      });
      alert('Pesanan berhasil ditolak dengan alasan.');
    } else {
  ...
  // ─── Tandai Sudah Jadi (Kesiapan Dapur) ─────────────────────────────────────
            ...
            <div className="w-screen max-w-md bg-[#F9F6EE] shadow-2xl flex flex-col justify-between border-l border-[#1A1A1A]/10">
              ...
              <div className="flex-1 overflow-y-auto px-6 py-5 space-y-6">
                ...
                <div className="bg-white rounded-2xl p-4 border border-[#1A1A1A]/5">
                  ...
                  {selectedOrder.receipt_path && selectedOrder.status !== 'rejected' ? (
                    <div className="space-y-3">
                    ...
                    <div className="p-3 bg-rose-50 border border-rose-200 rounded-xl text-rose-700 text-xs">
                      <p className="font-bold">❌ Ditolak dengan Alasan:</p>
                      <p className="font-sans mt-1 leading-relaxed">{selectedOrder.receipt_path}</p>
                    </div>
                  ) : (
                  ...
                  <div className="flex gap-2">
                    ...
                    {selectedOrder.status === 'pending' && (
                      <div className="flex gap-2 w-full">
                        ...
                        <button
                          onClick={() => handleRejectPrompt(selectedOrder.id)}
                          disabled={confirmingId === selectedOrder.id}
