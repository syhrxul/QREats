  ...
  return (
    <div className="bg-slate-50 min-h-screen font-sans">
      ...
      <div className="border-b border-slate-900/10 px-6 py-4 flex items-center justify-between bg-white">
        <div>
          <h2 className="text-lg font-bold text-slate-900">Kelola Pengguna Platform</h2>
          <p className="text-xs text-slate-900/40 mt-0.5">Daftarkan akun kasir, owner baru, dan pantau profil pengguna aktif</p>
        </div>
        <button
          onClick={fetchUsersAndShops}
          className="px-4 py-2 border border-slate-900/20 hover:bg-slate-900/5 text-xs font-bold rounded-xl transition-all"
        >
          🔄 Refresh
        </button>
      </div>
      ...
      <main className="max-w-5xl mx-auto px-6 py-8 space-y-6">
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
          ...
          {/* Form Add User */}
          <div className="bg-white border border-slate-900/8 rounded-xl p-6 shadow-[4px_4px_0px_0px_rgba(15,23,42,1)] border-2 border-slate-900 space-y-4 self-start">
            <div>
              <h3 className="font-bold text-slate-900 text-base">Registrasi User Baru</h3>
              <p className="text-xs text-slate-900/40 mt-0.5">Buat akun untuk pemilik kafe atau karyawan staf</p>
            </div>
            ...
            <form onSubmit={handleAddUser} className="space-y-3.5 text-xs">
              <div>
                <label className="block font-bold text-slate-900/60 uppercase tracking-wider mb-1">Nama Lengkap</label>
                <input
                  type="text"
                  required
                  value={newName}
                  ...
                  className="w-full px-3 py-2 bg-slate-50 border border-slate-900/10 rounded-lg focus:outline-none text-slate-900"
                />
              </div>

              <div>
                <label className="block font-bold text-slate-900/60 uppercase tracking-wider mb-1">Email</label>
                <input
                  type="email"
                  required
                  value={newEmail}
              ...
              <div>
                <label className="block font-bold text-slate-900/60 uppercase tracking-wider mb-1">Password</label>
                <input
                  type="password"
                  required
                  value={newPassword}
              ...
              <div className="grid grid-cols-2 gap-2">
                <div>
                  <label className="block font-bold text-slate-900/60 uppercase tracking-wider mb-1">Role Akun</label>
                  <select
                    value={newRole}
                    onChange={(e) => setNewRole(e.target.value as any)}
                    className="w-full px-2 py-2 bg-slate-50 border border-slate-900/10 rounded-lg focus:outline-none font-bold text-slate-900"
                  >
                ...
                <div>
                  <label className="block font-bold text-slate-900/60 uppercase tracking-wider mb-1">Pilih Toko</label>
                  <select
                    value={newShopId}
              ...
              <button
                ...
                className="w-full py-3 bg-slate-900 text-white font-bold rounded-xl hover:bg-[#333] transition-all disabled:opacity-50"
              >
          ...
          {/* Table Users */}
          <div className="lg:col-span-2 bg-white border border-slate-900/8 rounded-xl p-6 shadow-[4px_4px_0px_0px_rgba(15,23,42,1)] border-2 border-slate-900 space-y-4">
            <div>
              <h3 className="font-bold text-slate-900 text-base">Daftar Akun Pengguna</h3>
              <p className="text-xs text-slate-900/40 mt-0.5">Daftar email, role, dan relasi merchant di platform</p>
            </div>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
          ...
          <div className="relative bg-[#F9F6EE] rounded-xl overflow-hidden max-w-md w-full shadow-[4px_4px_0px_0px_rgba(15,23,42,1)] border-2 border-slate-900 border border-slate-900/10 p-6 space-y-6">
            <div className="flex items-center justify-between border-b border-slate-900/10 pb-4">
              <div>
                <h3 className="font-bold text-slate-900 text-lg font-sans">Edit Profil Pengguna</h3>
                <p className="text-xs text-slate-900/40 font-mono mt-0.5">ID: {editingUser.id}</p>
              </div>
              <button onClick={() => setEditingUser(null)} className="text-xl text-slate-900/40 hover:text-slate-900 cursor-pointer">
                ✕
              </button>
            </div>
            ...
            <form onSubmit={handleUpdateUser} className="space-y-4 text-xs">
              <div>
                <label className="block font-bold text-slate-900/60 uppercase tracking-wider mb-1 font-sans">Email</label>
                <input
                  type="email"
                  required
                  value={editEmail}
                  onChange={(e) => setEditEmail(e.target.value)}
                  className="w-full px-3 py-2.5 bg-white border border-slate-900/10 rounded-lg focus:outline-none font-bold text-slate-900 text-xs font-sans"
                />
              </div>

              <div>
                <label className="block font-bold text-slate-900/60 uppercase tracking-wider mb-1 font-sans">Password Baru</label>
                <input
                  type="password"
                  value={editPassword}
                  onChange={(e) => setEditPassword(e.target.value)}
                  placeholder="Kosongkan jika tidak diubah"
                  className="w-full px-3 py-2.5 bg-white border border-slate-900/10 rounded-lg focus:outline-none text-slate-900 text-xs font-sans"
                />
                <p className="text-[9px] text-slate-900/35 mt-1 font-sans">Catatan: Mengubah password akun user lain membutuhkan Admin API (Service Role Key).</p>
              </div>

              <div className="grid grid-cols-2 gap-3">
                <div>
                  <label className="block font-bold text-slate-900/60 uppercase tracking-wider mb-1 font-sans">Role Akun</label>
                  <select
                    value={editRole}
                    onChange={(e) => setEditRole(e.target.value as any)}
                    className="w-full px-2.5 py-2.5 bg-white border border-slate-900/10 rounded-lg focus:outline-none font-bold text-slate-900 font-sans"
                  >
                    <option value="owner">Owner</option>
                    <option value="admin">Admin</option>
                    <option value="kasir">Kasir</option>
                ...
                <div>
                  <label className="block font-bold text-slate-900/60 uppercase tracking-wider mb-1 font-sans">Hubungkan Toko</label>
                  <select
                    value={editShopId}
              ...
              <div className="flex gap-2 pt-2">
                ...
                <button
                  type="submit"
                  disabled={updating}
                  className="flex-1 py-3 bg-slate-900 text-white font-bold rounded-xl hover:bg-[#333] transition-all active:scale-[0.98] cursor-pointer font-sans"
                >
