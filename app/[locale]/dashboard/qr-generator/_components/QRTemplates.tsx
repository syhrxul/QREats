  ...
  switch (templateId) {
    case 'vintage':
      return (
        <div
          ...
          className="bg-[#FAF6EE] border-4 double border-[#5C4033] rounded-xl p-6 flex flex-col items-center justify-between text-[#5C4033] w-full min-h-[360px] shadow-[4px_4px_0px_0px_rgba(15,23,42,1)] border-2 border-slate-900 relative qr-card-print group hover:shadow-[4px_4px_0px_0px_rgba(15,23,42,1)] border-2 border-slate-900 transition-all duration-300"
        >
          <div className="absolute inset-1 border border-[#5C4033]/20 rounded-xl pointer-events-none" />
          ...
          <div className="p-3 bg-[#FAF6EE] border-2 border-[#5C4033] rounded-xl my-3">
          ...
          <div className="text-center w-full pb-1">
            ...
            <p className="text-3xl font-bold font-serif mt-0.5">{number ? `No. ${number}` : 'Meja'}</p>
    ...
    case 'dark':
      return (
        <div
          id={`qr-card-${table.name}`}
          className="bg-slate-900 border-2 border-[#D4AF37] rounded-xl p-6 flex flex-col items-center justify-between text-white w-full min-h-[360px] shadow-[4px_4px_0px_0px_rgba(15,23,42,1)] border-2 border-slate-900 relative qr-card-print group hover:shadow-[4px_4px_0px_0px_rgba(15,23,42,1)] border-2 border-slate-900 transition-all duration-300"
        >
          <div className="text-center w-full">
            <p className="text-[9px] font-bold tracking-widest text-[#D4AF37] uppercase">PREMIUM SELF ORDER</p>
            <h3 className="text-xs font-bold tracking-tight mt-1 truncate uppercase">{shopName || 'QREats Venue'}</h3>
          </div>
          <div className="p-3 bg-white rounded-xl my-4 ring-2 ring-[#D4AF37]/30">
    ...
    case 'tent':
      return (
        <div
          id={`qr-card-${table.name}`}
          className="bg-white border-2 border-dashed border-gray-300 rounded-xl p-6 flex flex-col items-center justify-between text-slate-900 w-full min-h-[480px] shadow-[4px_4px_0px_0px_rgba(15,23,42,1)] border-2 border-slate-900 relative qr-card-print group hover:shadow-[4px_4px_0px_0px_rgba(15,23,42,1)] border-2 border-slate-900 transition-all duration-300"
        >
          ...
          <div className="w-full text-center flex flex-col items-center justify-center transform rotate-180 h-1/2 pb-6 border-b border-gray-100/50">
            <h3 className="text-sm font-bold tracking-tight mb-1 truncate text-gray-600 uppercase">{shopName || 'QREats Cafe'}</h3>
          ...
          <div className="w-full text-center flex flex-col items-center justify-between h-1/2 pt-6">
            <div className="p-2.5 bg-[#FAF6EE] border border-gray-200 rounded-xl mx-auto w-fit">
            ...
            <div className="mt-2">
              ...
              <p className="text-2xl font-bold tracking-tight text-black mt-0.5">{number ? `Meja ${number}` : 'Meja'}</p>
              <a
                href={`/order/${table.token}`}
                target="_blank"
                rel="noopener noreferrer"
    ...
    default:
      return (
        <div
          id={`qr-card-${table.name}`}
          className="bg-white border border-slate-900/10 rounded-xl p-6 flex flex-col items-center justify-between text-slate-900 w-full min-h-[360px] shadow-[4px_4px_0px_0px_rgba(15,23,42,1)] border-2 border-slate-900 relative qr-card-print group hover:shadow-[4px_4px_0px_0px_rgba(15,23,42,1)] border-2 border-slate-900 transition-all duration-300"
        >
          <div className="text-center w-full">
            <h3 className="text-sm font-bold tracking-tight mt-1 truncate uppercase">{shopName || 'QREats Cafe'}</h3>
          </div>
          <div className="p-2.5 bg-slate-50 rounded-xl my-3">
            <QRCodeSVG value={url} size={130} bgColor="#F5F2EB" fgColor="#1A1A1A" level="H" />
          </div>
          <div className="text-center w-full">
            <p className="text-xs font-bold text-slate-900/40 uppercase tracking-wider">{namePrefix || 'Table'}</p>
            <p className="text-4xl font-bold tracking-tighter mt-1">{number ? `#${number}` : 'Meja'}</p>
            <p className="text-[9px] text-slate-900/30 uppercase tracking-widest font-bold mt-2.5 print:hidden">Scan to Order</p>
            <a
              href={`/order/${table.token}`}
              target="_blank"
              rel="noopener noreferrer"
